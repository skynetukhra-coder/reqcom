...
if (isset($_POST['forward'])) {
    ...
    $sql_insert = "INSERT INTO requisitions 
        (section_id, item_id, model_id, quantity, requested_by, assigned_bo, section_forward, bo_approve, approve_date)
        VALUES (NULL, ?, ?, ?, ?, ?, 'Y', 'F', CURDATE())";
    ...
    if($stmt_insert->execute()){
        echo "<script>alert('Self requisition forwarded to ITSC'); window.location='bo_home.php';</script>";
        exit();
    } else {
...
<title>BO Self Requisition</title>
...
.dashboard-card{ background:#fff; border:1px solid #dbe2ea; border-radius:14px; padding:22px; }
.page-title{ font-size:22px; font-weight:700; color:#1a3a6b; margin-bottom:25px; display:flex; align-items:center; gap:10px; }
...
.submit-btn{ border:none; background: linear-gradient(135deg, #1a3a6b, #102544); color:#fff; padding:13px 22px; border-radius:12px; font-size:14px; font-weight:600; cursor:pointer; display:inline-flex; align-items:center; gap:8px; margin-top:10px; }
    ...
    <div class="letterhead">
        ...
        <div class="content">
            <div class="session-bar">
                ...
                <a href="bo_home.php" class="back-btn">
                    <i class="ti ti-arrow-left"></i> Back to Dashboard
                </a>
            </div>

            <div class="dashboard-card">
                <div class="page-title"><i class="ti ti-file-plus"></i> BO Self Requisition</div>

                <form method="POST">
                    <div class="form-grid">
                        <div class="input-group">
                            <label>Branch Officer</label>
                            <input type="text" value="<?php echo htmlspecialchars($_SESSION['userid']); ?>" readonly>
                        </div>
                    ...
                    <button type="submit" name="forward" class="submit-btn">
                        <i class="ti ti-send"></i> Forward to ITSC
                    </button>
                </form>
            </div>
